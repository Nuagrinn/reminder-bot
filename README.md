...
действиями `Готово`, `Перенести` и `Удалить`. Если событий нет, бот явно пишет
`На сегодня событий нет` или похожее пустое состояние. `Ближайшие` включает
незакрытые события начиная с сегодняшнего дня, поэтому all-day задачи не
пропадают вечером из-за внутреннего утреннего якоря.
