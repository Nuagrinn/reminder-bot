...
def build_system_prompt(request: ReminderParseRequest) -> str:
    return (
        ...
        "date/time, temporal profile, recurrence, optional notification offset, "
        "optional context links/locations, and optional structured shopping "
        "list content. Return only "
        ...
        "- broad day parts such as Russian morning/day/evening/night are not exact "
        "clock times; do not convert them to 09:00, 14:00, or 20:00 unless the "
        "user gave an explicit numeric time;\n"
        ...
        "- title must omit URLs, addresses, and helper phrases such as 'link', "
        "'url', 'ssylka', 'address', 'adres';\n"
        "- preserve links exactly in context.links[].url and use short labels "
        "like Telemost, Zoom, Google Meet, Docs, Ticket;\n"
        "- explicit addresses go to context.locations[].address with a short label;\n"
        "- shopping lists are still broad event_type='task'; do not create a new "
        "event_type for them;\n"
        "- if the user asks to buy several products, fill content.kind='shopping_list' "
        "and content.shopping_list.items[];\n"
        "- shopping item title must contain only the product/item name; put amount "
        "like '2 l' or '10 pcs' into quantity when it is clear, otherwise quantity='';\n"
        "- if a shopping list has no explicit date/time, use today as an all-day "
        "day_task instead of asking clarification;\n"
        ...
        "- Russian shopping list '\\u043a\\u0443\\u043f\\u0438\\u0442\\u044c "
        "\\u043c\\u043e\\u043b\\u043e\\u043a\\u043e, \\u0445\\u043b\\u0435\\u0431, "
        "\\u044f\\u0439\\u0446\\u0430' -> title='\\u041f\\u043e\\u043a\\u0443\\u043f\\u043a\\u0438', "
        "event_type='task', temporal_profile='day_task', date=today, and "
